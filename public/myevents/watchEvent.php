...
// Toggling a watch changes data, so require a CSRF-protected POST.
requireCsrf();

if ($currentPersonId > 0 && strlen($_POST["event"]) > 6) {
    ...
    $eventWatch->bind_param("si", $_POST["event"], $currentPersonId);
    $eventWatch->execute();
    ...
    if ($eventWatchData[0]["count"] > 0) {
        ...
        $watchSubmissionDelete->bind_param("si", $_POST["event"], $currentPersonId);
        $watchSubmissionDelete->execute();

        echo('<a class="btn btn-outline-secondary watch-toggle float-start" data-event="' . e($_POST["event"]) . '" title="Add this to events I\'m watching"><i class="fa-solid fa-eye"></i></a>');
        ...
        $teamMatchCreate->bind_param("s", $_POST["event"]);
        ...
        $teamMatch->bind_param("s", $_POST["event"]);
        $teamMatch->execute();
        ...
        echo('<a class="btn btn-outline-warning watch-toggle float-start" data-event="' . e($_POST["event"]) . '" title="Remove this event from my watch list"><i class="fa-solid fa-eye"></i></a>');
    }
